...
void sortmass(int *p, int startnum, int endnum)
{
    ...
    // повторяем сортировку сужая диапазон
...
{
    // ищем скобки () [] {}
    // на открытую "+" в соответствующий индекс, на закрытую "-"
    // в конце все индексы должны быть 0
    int krug = 0; // индекс круглых
    int kvad = 0; // квадратных
    int fig = 0;  // фигурных
    ...
    return "Ñêîáêè: ( - " + (krug == 0?"закрыта ":"незакрыта") +
